...
/**
 * Récupère le token MyMinfin déchiffré pour un mandant + valide son expiry.
 * Pattern dupliqué de fpsAuth.service.js#refreshMandantByEcb — candidat à
 * factoriser dans un helper partagé plus tard.
 */
async function loadValidAccessToken(ecbNumber) {
  ...
  if (!mandant.access_token_encrypted) {
    throw new Error(`no_valid_token: missing access_token for ECB ${ecbNumber}`);
  }

  const expiry = mandant.token_expiry ? new Date(mandant.token_expiry).getTime() : 0;
  if (!expiry || expiry <= Date.now()) {
    throw new Error(`no_valid_token: access_token expired for ECB ${ecbNumber}`);
  }
